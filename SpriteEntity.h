...
private:
    // const-agnostic code
    template <class This> requires std::is_pointer_v<This>
    [[nodiscard]] static auto& get_sprite(This self, Direction direction, State state) {
        switch (state) {
            using enum State;
...
public:
    SpriteEntity(const gfx::Window& window, gfx::Vec position)
